...
//17285-AR-223134398
wpniAds.templates.initRule('marketing336x90');
wpniAds.templates.rules.marketing336x90 = {
  what:['336x90'],
  where:[''],
  when:['201110250000/201112312359']
}
...
wpniAds.templates.rules.agoogleaday = {
  what:['agoogleaday'],
  where:['arts','life','technology','science'],
  when:['201201190000/201203312359']
}

//CD
wpniAds.templates.initRule('leadershipdataviz_featurebar');
wpniAds.templates.rules.leadershipdataviz_featurebar = {
  what:['featurebar'],
  where:['business/leadershipdataviz'],
  when:['201111130000/201112312359']
}

//17872-KZ-247985008, 247984991
wpniAds.templates.initRule('hp_takeover');
wpniAds.templates.rules.hp_takeover = {
  what:['leaderboard','!tiffanytile'],
  where:['homepage'],
  when:['201112060000/201112062359;201112120000/201112122359;201112140000/201112142359']
}

//18035-JH
wpniAds.templates.initRule('starbucks_sponsor');
wpniAds.templates.rules.starbucks_sponsor = {
  what:['sponsor'],
  where:['life/holidays'],
  when:['201112130000/201112252359']
}

//18129-KZ-251118889, 251118893
wpniAds.templates.initRule('hpLeaderboard');
wpniAds.templates.rules.hpLeaderboard = {
  what:['leaderboard','!tiffanytile'],
  where:['homepage'],
  when:['201112220000/201112221200']
}
...
wpniAds.templates.rules.hivesponsor = {
  what:['sponsor', 'featurebar', 'featurebar_2'],
  where:['business/inventyourfuture'],
  when:['201112190000/201201112359']
}

//18301-AR
wpniAds.templates.initRule('hbopush');
wpniAds.templates.rules.hbopush = {
  what:['pushdown'],
  where:['homepage'],
  when:['201201200000/201201202359']
}

//18327-JH
wpniAds.templates.initRule('myslate120x60');
wpniAds.templates.rules.myslate120x60 = {
  what:['120x60','120x60_2'],
  where:['myslate'],
  when:['201201200000/201203312359']
}

//MIDNIGHT FIX
wpniAds.templates.initRule('homepagePushdown');
wpniAds.templates.rules.homepagePushdown = {
  what:['pushdown'],
  where:['homepage'],
  when:['201201240000/201201242359']
}

//18159-KZ
/*wpniAds.templates.initRule('kindlepushdown');
wpniAds.templates.rules.kindlepushdown = {
  what:['pushdown'],
  where:['homepage'],
  when:['201112290000/201112292359']
}*/

//17503-RN
/*wpniAds.templates.initRule('liberty_mutual_lb');
wpniAds.templates.rules.liberty_mutual_lb = {
  what:['leaderboard','!tiffanytile'],
  where:['homepage'],
  when:['201111080000/201111082359;201111100000/201111102359']
}*/
...
//17912-ST
wpniAds.templates.initRule('samsung_ll_bb');
wpniAds.templates.rules.samsung_ll_bb = {
  what: ['bigbox','leaderboard','!tiffanytile'],
  where: ['homepage'],
  when: ['201112070000/201112082359']
}

//18014-ST
wpniAds.templates.initRule('samsung_ll_bb2');
wpniAds.templates.rules.samsung_ll_bb2 = {
  what: ['bigbox','leaderboard','!tiffanytile'],
  where: ['homepage'],
  when: ['201112130000/201112132359']
}

//18243-AR
wpniAds.templates.initRule('hbo_opa_tt');
wpniAds.templates.rules.hbo_opa_tt = {
  what: ['pushdown', 'tiffanytile'],
  where: ['homepage'],
  when: ['201201130000/201201132359']
}

//18291-KZ
wpniAds.templates.initRule('slate_lb_bb');
wpniAds.templates.rules.slate_lb_tt = {
  what: ['leaderboard', 'bigbox', '!tiffanytile'],
  where: ['homepage'],
  when: ['201201180000/201201182359']
}
...
wpad.getmetakw = function(){
  ...
  var obj = {// obj is set up this way so that we can easily add to it if necessary
    '!c' : {
      ...
      human_disaster : ['sex abuse','aground', 'rescue', 'attack', 'disaster', 'explosion', 'war', 'hostage', 'terror', 'terrorist', 'bomb', 'blast', 'mining', 'miner', 'violence', 'riot', 'plane crash', '9/11', 'sept. 11', 'september 11'],
